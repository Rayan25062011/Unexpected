...
def main():
 ...
 """.format(GNSL, REDL, WHSL))
    ...
    if option == '12':
        os.system("git clone https://github.com/Rayan25062011/Unexpected && chmod +x Unexpected.py && python3 Unexpected.py")
        option = input(ENDL + "Unexpected"+GNSL+"("+REDL + "main_menu" + GNSL + ")"+ENDL + "> ")
    ...
    if option == '14':
        print(f"[{GNSL}+{ENDL}] Go to{GREEN}Github{ENDL} in the issue section of my program and report a bug")
        option = input(ENDL + "Unexpected"+GNSL+"("+REDL + "main_menu" + GNSL + ")"+ENDL + "> ")
    ...
    if option == '99':
        ...
        if "adb" not in adbCMD:
            print(f"[{REDL}-{ENDL}] Command: 'adb' is not in: {adbCMD}")
        else:
            os.system(adbCMD)
